    ...
    render() {
      return (
        <React.Fragment>
          ...
          {
            this.state.schedule == null
            ? <Row style={{ height: "40px", borderBottom: "solid 1px #333333", lineHeight: "40px" }}><Col span="12">Loading...</Col></Row>
            : this.state.finishedTalks.map((item, i) => {
              ...
              return (
                <Row key={`item-${i}`} style={{ height: "40px", borderBottom: "solid 1px #333333", lineHeight: "40px" }}>
                  ...
                  <Col span="1">{notesFile ? <Link to={`/notes/${item["Date"]}`}>Notes</Link> : ""}</Col>
                </Row>                
              )
            })
          }
